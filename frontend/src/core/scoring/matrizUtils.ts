...
export function getResultados(): string[] {
  return Array.from(new Set(matrizData.map((item: MatrizItem) => item.Resultado)));
}

export function getGolpes(resultados: string[]): string[] {
  return Array.from(new Set(
    matrizData.filter((item: MatrizItem) => resultados.includes(item.Resultado)).map((item: MatrizItem) => item.Golpe)
  ));
}

export function getEfeitos(resultados: string[], golpes: string[]): string[] {
  return Array.from(new Set(
    matrizData.filter((item: MatrizItem) => resultados.includes(item.Resultado) && golpes.includes(item.Golpe)).map((item: MatrizItem) => item.Efeito)
  ));
}

export function getDirecoes(resultados: string[], golpes: string[], efeitos: string[]): string[] {
  return Array.from(new Set(
    matrizData.filter((item: MatrizItem) => resultados.includes(item.Resultado) && golpes.includes(item.Golpe) && efeitos.includes(item.Efeito)).map((item: MatrizItem) => item.Direcao)
  ));
}
